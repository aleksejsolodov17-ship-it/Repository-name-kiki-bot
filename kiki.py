...
# Глобальный клиент для мгновенного ответа
...
def db_query(sql, params=(), is_select=False):
    with sqlite3.connect("kiki.db", check_same_thread=False, timeout=10) as conn:
        cursor = conn.execute(sql, params)
        return cursor.fetchall() if is_select else conn.commit()

# =========================
# 🧠 МОЗГ GIGACHAT (Оптимизированный)
# =========================
def ask_ai(user_id, text, name):
    try:
        sys_prompt = f"Ты KiKi — теплый ИИ-психолог. Пользователя зовут {name}. Отвечай кратко, эмпатично 🌿."
        rows = db_query("SELECT role, content FROM memory WHERE user_id = ? ORDER BY timestamp DESC LIMIT 4", (user_id,), True)
        ...
        # Сохранение памяти в фоне
        threading.Thread(target=lambda: db_query("INSERT INTO memory VALUES (?, ?, ?, ?)", (user_id, 'user', text, datetime.datetime.now()))).start()
        threading.Thread(target=lambda: db_query("INSERT INTO memory VALUES (?, ?, ?, ?)", (user_id, 'assistant', ans, datetime.datetime.now()))).start()
        return ans
...
def create_chart(user_id):
    ...
    plt.ylim(0, 105); plt.title("Твой прогресс 🌿", color=color)
...
async def process_update(update: Update):
    ...
    if state == "naming":
        ...
        await bot.send_message(user_id, f"Рада познакомиться, {new_name}! 😊", reply_markup=MAIN_KB)
        return
    ...
    if text == "🧠 Тест" or state == "testing":
        if text == "🧠 Тест":
            ...
            if new_step < len(TEST_QUESTIONS):
                ...
                await bot.send_message(user_id, f"Твой уровень благополучия: {final}% ✨", reply_markup=MAIN_KB)
        return
    ...
    if text == "📊 Аналитика":
        ...
        else: await bot.send_message(user_id, "Пройди тест! 🧠", reply_markup=MAIN_KB)
        return

    if text == "🎨 Тема":
        new_t = 'dark' if db_query("SELECT theme FROM users WHERE user_id=?", (user_id,), True)[0][0] == 'light' else 'light'
...
def webhook():
    threading.Thread(target=lambda: asyncio.run(process_update(Update.de_json(request.get_json(force=True), bot)))).start()
    return "ok", 200
