...
with sqlite3.connect("kiki.db") as conn:
    conn.execute("CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, name TEXT, state TEXT DEFAULT 'idle')")
...
def ask_ai(user_id, text, name):
    try:
        # Прямое подключение без лишних оберток
        with GigaChat(credentials=GIGA_KEY, verify_ssl_certs=False, scope="GIGACHAT_API_PERS") as giga:
            sys_prompt = f"Ты KiKi — эмпатичная девушка. Твой друг: {name}. Говори СТРОГО в женском роде 🌿."
            
            # Берем только 3 последних сообщения для экономии памяти
        ...
        print(f"AI ERROR: {e}")
        return "Я здесь, Exvill! 🌿 Просто настраиваюсь на твою волну. Что у тебя нового?"
...
async def handle_msg(update: Update):
    ...
    # Обычный AI чат (без очередей для стабильности)
...
def webhook():
    # Используем простейший тред
    data = request.get_json(force=True)
    t = threading.Thread(target=lambda: asyncio.run(handle_msg(Update.de_json(data, bot))))
    t.start()
    return "ok", 200
